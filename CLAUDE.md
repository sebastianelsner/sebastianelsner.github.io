...
6. Generate the per-post OG card and update the post's `og:image` meta:

   ```bash
   bin/og-card "Post Title" my-post "Month DD, YYYY"
   ```

   This writes `assets/img/og-my-post.png` (1200×630). Then update the post's head:
   ...
   Requires `inkscape` on `PATH`. The script reads no other dependencies.

## OG cards

- Site-level pages (root `index.html`, `blog/index.html`) use `assets/img/og.png` — the brand card with name, manifesto, and pillars.
- Each blog post has its own `assets/img/og-<slug>.png` generated by `bin/og-card`. Title is centred and auto-wraps up to 3 lines (32 chars/line).
- The brand card source is `/tmp/og-card.svg` (regenerate by editing the SVG and running `inkscape ... --export-filename=assets/img/og.png`). The post-card template lives inside `bin/og-card`.
